...
namespace Yurowm.EditorCore {
    ...
    public class MetaWindow<W> : EditorWindow where W : IMetaEditor {
        ...
        internal W editor = default(W);

        void OnEnable() {
            Type targetType = typeof(W);
            if (!targetType.IsAbstract && !targetType.IsInterface) {
                editor = Activator.CreateInstance<W>();
    ...
    public static class EUtils {
        ...
        public static List<FileInfo> SearchFiles(string directoryPath) {
            ...
            if (directory.Exists) {
                result.AddRange(directory.GetFiles().ToList());
        ...
        public static string CombinePath(params string[] paths) {
            ...
            if (paths.Length == 0) throw new IndexOutOfRangeException("paths");
    ...
    internal class AsyncExecutor {
        ...
        public AsyncExecutor(Action action, Action onSuccess = null, Action<Exception> onFailed = null, float delay = 0) {
            this.action = action;
        ...
        void Start(object state) {
            ...
            complete = true;
            if (action == null)
                onFailed(new NullReferenceException("action"));
        }

        public bool IsComplete() {
            if (!complete && expiryTime.HasValue && expiryTime.Value <= DateTime.Now)
                complete = true;
            return complete;
        }
    }
    ...
    public static class GUIHelper {
        ...
        public class Scroll {
            ...
            public ScrollDisposable Start() {
                return new ScrollDisposable(ref _position, style, options);
            }
            ...
            public void ScrollTo(float x = 0, float y = 0) {
                _position.x = x;
                _position.y = y;
            }
    ...
    public static class EditorIcons {
        ...
        static Dictionary<string, Texture2D> bi_icons = new Dictionary<string, Texture2D>();
        ...
        static Texture2D FindIcon(string name) {
            return EditorGUIUtility.Load(string.Format("LevelEditor/{0}.png", name)) as Texture2D;
        }
    }
}
