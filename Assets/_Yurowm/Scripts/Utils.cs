...
namespace Yurowm.GameCore {
    ...
    public class int3 {
        ...
        public static int3 Parse(string raw) {
            if (parser.IsMatch(raw)) {
                Match match = parser.Match(raw);
                int3 result = new int3(int.Parse(match.Groups["x"].Value), int.Parse(match.Groups["y"].Value), int.Parse(match.Groups["z"].Value));
                return result;
            }
            throw new FormatException("Can't to parse \"" + raw + "\" to int3 format. It must have next format: (int,int,int)");
        }
    ...
    public class DelayedAccess {
        ...
        public DelayedAccess(float delay, bool fromThisMoment = false) {
            timeSpan = new TimeSpan(0, 0, 0, 0, Mathf.RoundToInt(delay * 1000));
